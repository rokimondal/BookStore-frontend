...
const UserDashboard = () => {
    ...
    const { data: orders = [], isLoading, isError, error } = useGetOrdersByEmailQuery(currentUser?.email);

    if (isLoading) return <div className="spinner">Loading...</div>;
    if (isError) return <div>Error getting orders data: {error.message}</div>;
                        ...
                        <ul className="space-y-4">
                            {orders.map((order) => (
                                <li key={order._id} className="bg-gray-50 p-4 rounded-lg shadow-sm space-y-1">
                                    ...
                                    {Array.isArray(order.productIds) && order.productIds.map((productId) => (
